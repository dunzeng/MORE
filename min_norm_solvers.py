...
class MinNormSolver:
    ...
    def _min_norm_2d_accelerated(vecs, dps):
        dmin = 1e8
        # V2
        vec = torch.stack(vecs)
        M = torch.matmul(vec, vec.T)
        # print("-------debug---------", len(vecs))
        for i in range(len(vecs)):
            if (i,i) not in dps:
                dps[(i,i)] = M[i,i].item()
            for j in range(i+1,len(vecs)):
                if (j,j) not in dps:
                    dps[(j,j)] = M[j,j].item()
                if (i,j) not in dps:
                    dps[(i,j)] = dps[(j,i)] = M[i,j].item()
                
                c,d = MinNormSolver._min_norm_element_from2(dps[(i,i)], dps[(i,j)], dps[(j,j)])
                # print("------debug--------- i-j {}-{}, d {}, dmin {}".format(i, j, d, dmin))
                if d < dmin:
                    dmin = d
                    sol = [(i,j),c,d]
    ...
    def find_min_norm_element_FW(vecs):
        ...
        dps = {}
        # init_sol, dps = MinNormSolver._min_norm_2d(vecs, dps)
        init_sol, dps = MinNormSolver._min_norm_2d_accelerated(vecs, dps)
...
def gradient_normalizers(grads, losses=None, normalization_type='l2'):
    gn = {}
